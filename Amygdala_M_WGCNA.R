...
datExpr_M <- read_csv("./DESeq2-Amygdala-M-fullmodel.csv")
...
datTraits_M <- read_csv("./data/datTraits_Amygdala_M.csv", 
...
MEs$Group <- Amy_design_M$Group
MEs$LG_P1_5_scaled <- Amy_design_M$LG_P1_5_scaled
MEs$Nest_attendance_P1_5_scaled <- Amy_design_M$Nest_attendance_P1_5_scaled
...
moduleTraitLm = lm(MEblue ~ Group + Nest_attendance_P1_5_scaled + Group:Nest_attendance_P1_5_scaled, data = MEs)
moduleTraitLm = lm(MEgreen ~ Group + Nest_attendance_P1_5_scaled + Group:Nest_attendance_P1_5_scaled, data = MEs)

summary(moduleTraitLm)


#Exporting gene names within modules as a txt file
bluemodule<-as.data.frame(names(datExpr_M_log)[moduleColors=="blue"])
greenwmodule<-as.data.frame(names(datExpr_M_log)[moduleColors=="green"])

write_delim(bluemodule, file = "M-Amygdala-bluemodule.txt", delim = "\n")
write_delim(greenmodule, file = "M-Amygdala-greenmodule.txt", delim = "\n")

allgenes<-as.data.frame(names(datExpr_M_log)) #all genes analyzed for WGCNA
write_delim(allgenes, file = "M-Amygdala-allgenes.txt", delim = "\n")
...
WGCNA_Amy_M <- ggplot(MEs, aes(x = LG_P1_5_scaled, y = MEturquoise, color = Group)) +
