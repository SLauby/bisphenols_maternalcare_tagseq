...
Amy_tagseq_F <- read_excel("./data/Amygdala_tagseq_counts_F.xlsx")
View(Amy_tagseq_F)
...
Amy_tagseq_F_matrix <- Amy_tagseq_F[ , -1]
row.names(Amy_tagseq_F_matrix) <- Amy_tagseq_F$Gene_ID
...
ddsMatrix <- DESeqDataSetFromMatrix(
  countData = Amy_tagseq_F_matrix, 
  colData = Amy_design_F, 
  design = ~ 
    Group+
...
write.csv(normalized_counts, file="DESeq2-Amygdala-F-fullmodel.csv", quote=FALSE)
...
plotExpr_F$Group <- Amy_design_F$Group
plotExpr_F$LG_P1_5_scaled <- Amy_design_F$LG_P1_5_scaled
plotExpr_F$Nest_attendance_P1_5_scaled <- Amy_design_F$Nest_attendance_P1_5_scaled
...
MBxBP_Amy_F <- ggplot(plotExpr_F, aes(x = Nest_attendance_P1_5_scaled, y = Esr1, color = Group)) +
