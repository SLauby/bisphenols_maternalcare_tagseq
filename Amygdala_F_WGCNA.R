...
datExpr_F <- read_csv("./DESeq2-Amygdala-F-fullmodel.csv")
...
datTraits_F <- read_csv("./data/datTraits_Amygdala_F.csv", 
...
Amy_design_F <- read_excel("./data/Amygdala_tagseq_variables_F.xlsx")
View(Amy_design_F)

#Factor prenatal BP groups, control group first
Amy_design_F$Group <- factor(Amy_design_F$Group, levels = c('Corn Oil', '50 μg/kg BPA', '50 μg/kg Mixed BP', '150 μg/kg Mixed BP'))

#Scaling maternal care measures so 0 is approximately the mean and 1 is approximately the stdev
Amy_design_F$Nest_attendance_P1_5_scaled <- (Amy_design_F$Nest_attendance_P1_5-1600)/400
Amy_design_F$LG_P1_5_scaled <- (Amy_design_F$LG_P1_5-300)/100
...
MEs$Group <- Amy_design_F$Group
MEs$LG_P1_5_scaled <- Amy_design_F$LG_P1_5_scaled
MEs$Nest_attendance_P1_5_scaled <- Amy_design_F$Nest_attendance_P1_5_scaled
...
moduleTraitLm = lm(MEblack ~ Group + LG_P1_5_scaled + Group:LG_P1_5_scaled, data = MEs)
...
blackmodule<-as.data.frame(names(datExpr_F_log)[moduleColors=="black"])
yellowmodule<-as.data.frame(names(datExpr_F_log)[moduleColors=="yellow"])

write_delim(blackmodule, file = "F-Amygdala-blackmodule.txt", delim = "\n")
write_delim(yellowmodule, file = "F-Amygdala-yellowmodule.txt", delim = "\n")

allgenes<-as.data.frame(names(datExpr_F_log)) #all genes analyzed for WGCNA
write_delim(allgenes, file = "F-Amygdala-allgenes.txt", delim = "\n")
...
WGCNA_Amy_F <- ggplot(MEs, aes(x = LG_P1_5_scaled, y = MEturquoise, color = Group)) +
