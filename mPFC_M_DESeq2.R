...
mPFC_tagseq_M <- read_excel("./data/mPFC_tagseq_counts_M.xlsx")
View(mPFC_tagseq_M)
...
mPFC_tagseq_M_matrix <- mPFC_tagseq_M[ , -1]
row.names(mPFC_tagseq_M_matrix) <- mPFC_tagseq_M$Gene_ID
...
ddsMatrix <- DESeqDataSetFromMatrix(
  countData = mPFC_tagseq_M_matrix, 
  colData = mPFC_design_M, 
  design = ~ 
    Group+
...
write.csv(normalized_counts, file="DESeq2-mPFC-M-fullmodel.csv", quote=FALSE)
...
plotExpr_M$Group <- mPFC_design_M$Group
plotExpr_M$LG_P1_5_scaled <- mPFC_design_M$LG_P1_5_scaled
plotExpr_M$Nest_attendance_P1_5_scaled <- mPFC_design_M$Nest_attendance_P1_5_scaled
...
MBxBP_mPFC_M <- ggplot(plotExpr_M, aes(x = Nest_attendance_P1_5_scaled, y = Esr1, color = Group)) +
